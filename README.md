...
* [Data And Tokenizer](#data-and-tokenizer)
* [Converting names to sequences and vice versa](#converting-names-to-sequences-and-vice-versa)
...
*Notes:*
  ...
  * The third line extracts the first 100 characters of data using the slicing operator and prints them to the console.
  *```with``` is a Python statement that is used to create a context in which a file or a resource is used. It is typically used along with the ```open()``` function to open a file and ensure that the file is closed when the block of code inside the with statement finishes executing.*
...
```
A tokenizer is used to give a numeric representation to all the
character items in our vocabulary so that they’re understood by
the model. 
...
The model is built using the Sequential API that allows for building a linear stack of layers in a neural network.
The model architecture consists of the following layers, in order:

* Embedding layer: 
This layer maps each character in the input sequence to a fixed-size vector of dimension 8. The ```max_number_of_chars``` parameter specifies the maximum number of distinct characters in the input. The ```input_length``` parameter specifies the length of the input sequence, which is ```max_len - 1```, where ```max_len``` is the maximum length of names in the dataset.
Note: it’s ```max_len - 1``` because that one is used to represent the label.
...
1. Optimizer: This parameter specifies the optimization algorithm used to update the weights of the neural network during training.
2. Loss: This parameter specifies the loss function that is used to measure the difference between the predicted output of the neural network and the true output during training. 
...
3. Metrics: This parameter specifies the evaluation metric used to monitor the performance of the neural network during training. 
